...
const LatestBookCard = ({ book }) => {
    const { _id, image, bookName, price, author,
        publisher, tags, category, rating, totalPages, yearOfPublishing } = book
    return (
        <div>
            <Link to={`/books/${_id}`}>
                <div className="card bg-base-100 w-96 shadow-xl p-8">
                    ...
                    <div className="card-body">
                        <div className='flex justify-start gap-4'>
                            {
                                tags.map((tag, idx) => <button key={idx} className="btn btn-xs bg-pink-800 text-white">{tag}</button>)
                            }
                        </div>
